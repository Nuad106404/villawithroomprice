...
export function BookingCard() {
  ...
  // Get base price and discounted price from database
  ...
  // Calculate price per night based on number of rooms selected
  const pricePerNight = basePrice - (maxRooms - rooms) * priceReductionPerRoom;
  
  // Apply discount if available
  const actualPricePerNight = discountedPrice > 0 
    ? (discountedPrice - (maxRooms - rooms) * priceReductionPerRoom)
    : pricePerNight;
  ...
  const totalPrice = totalNights * actualPricePerNight;

  // Format price displays
  const formattedPricePerNight = formatPrice(pricePerNight);
  const formattedDiscountedPrice = discountedPrice > 0 ? formatPrice(actualPricePerNight) : '';
  const formattedTotalPrice = formatPrice(totalPrice);

  // Calculate discount percentage if there's a discounted price
  const discountPercentage = discountedPrice > 0
    ? Math.round(((pricePerNight - actualPricePerNight) / pricePerNight) * 100)
    : 0;
                ...
                <div className="flex justify-between text-sm sm:text-base text-green-600">
                  ...
                  <span>
                    {new Intl.NumberFormat('en-US', {
                      ...
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
                    }).format((pricePerNight - actualPricePerNight) * totalNights)}
                  </span>
                </div>
              )}
